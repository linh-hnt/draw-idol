...
namespace SIS
{
...
#endif
    {
        ...
        /// </summary>
        public static event Action<string, string, Product> purchaseSucceededEvent;
...
#pragma warning restore 0649
        ...
        public void Initialize()
        {
            ...
            {
                OnInitializeFailed("NoLinkedGoogleAccount");
            });
            builder.Configure<IGooglePlayConfiguration>().SetDeferredPurchaseListener((Product p) =>
            {
                OnPurchaseFailed("Purchase of product " + p.metadata.localizedTitle + " is pending.");
            });
            ...
            appStore = !string.IsNullOrEmpty(customModule.appStore)
                ? customModule.appStore
                : standardModule.appStore.ToString();
...
#if UNITY_IOS
            extensions.GetExtension<IAppleExtensions>().RegisterPurchaseDeferredListener((Product p) =>
            {
                OnPurchaseFailed("Purchase of product " + p.metadata.localizedTitle + " is pending.");
            });
#endif
        ...
        private void OverwriteWithFetch()
        {
            foreach (Product p in controller.products.all)
            {
                ...
                {
                    IAPExchangeObject exchange =
                        product.priceList.Find(x => x.type == IAPExchangeObject.ExchangeType.RealMoney);
        ...
        public static void Purchase(string productID)
        {
            ...
            if (product.type == ProductType.NonConsumable && DBManager.GetPurchase(productID) > 0 &&
                !AFramework.FrameworkGlobalConfig.IsTester)
            {
            ...
            switch (product.IsVirtual())
            {
                case false:
                    ...
                    if (product.customBilling.active)
                        extensions.GetExtension<SISPurchasingExtension>()
                            .Purchase(p.definition, product.customBilling.provider);
                    else
                ...
                case true:
                    ...
                    GetInstance().CompletePurchase(product.ID, null);
                    break;
            }
        }
        ...
        public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs e)
        {
            ...
            {
                if (IAPManager.isDebug) Debug.Log("IAPManager receiptValidationPurchaseEvent");
                receiptValidationPurchaseEvent(e.purchasedProduct);
                return PurchaseProcessingResult.Pending;
            }
            else
            {
                CompletePurchase(e.purchasedProduct.definition.id, e.purchasedProduct.transactionID);
            }
#if USE_BYTE_BREW
#if UNITY_IOS
            ByteBrewSDK.ByteBrew.TrackInAppPurchaseEvent("Apple App Store", e.purchasedProduct.metadata.isoCurrencyCode,
                (float)e.purchasedProduct.metadata.localizedPrice, e.purchasedProduct.definition.storeSpecificId,
                e.purchasedProduct.transactionID);
            ByteBrewSDK.ByteBrew.TrackiOSInAppPurchaseEvent("Apple App Store",
                e.purchasedProduct.metadata.isoCurrencyCode, (float)e.purchasedProduct.metadata.localizedPrice,
                e.purchasedProduct.definition.storeSpecificId, e.purchasedProduct.transactionID,
        ...
        /// </summary>
        public void CompletePurchase(string productID, string transactionID, bool withEvent = true, Product item = null)
        {
            IAPProduct product = GetIAPProduct(productID);
                    ...
                    case IAPExchangeObject.ExchangeType.VirtualProduct:
                        ...
                        switch (exchange.product.type)
                        {
                            ...
                            case ProductType.Subscription:
                                if (!AFramework.FrameworkGlobalConfig.IsTester &&
                                    DBManager.GetPurchase(exchange.product.ID) > 0)
                                    return;
...
#if SIS_IAP
            if (AFramework.FrameworkGlobalConfig.IsTester && (Time.unscaledTime - initializeTime) <= 0.2f &&
                product.type != ProductType.Consumable) //restore purchase, does not apply for tester device
            {
                return;
            }
#endif
            ...
            if (withEvent)
            {
                purchaseSucceededEvent?.Invoke(product.ID, transactionID, item);
            }
        }
        ...
        public void OnInitializeFailed(string error)
        {
            switch (error)
            {
                case "NoLinkedGoogleAccount":
                    ...
                    Debug.LogWarning(
                        "Unable to connect to Google Play. User may not have a Google account on their device.");
                    break;
            }
        ...
        public void OnPurchaseFailed(Product item, PurchaseFailureReason reason)
        {
            ...
            if (reason == PurchaseFailureReason.DuplicateTransaction && item.definition.type != ProductType.Consumable)
            {
                purchaseSucceededEvent(item.definition.id, item.transactionID, item);
            }
            else
            {
        ...
        public static string GetNextUpgrade(string productId)
        {
            ...
            if (DBManager.GetPurchase(currentID) == 0 || product == null || product.nextUpgrade == null)
                return currentID;
        ...
        public static bool HasProductRewards(string productId)
        {
            ...
            if (product.rewardList.Exists(x => x.currency != null) ||
                product.rewardList.Exists(x => x.product != null && x.product.ID != productId))
                return true;
            else
                return false;
        }
        ...
        {
            return instance.asset.productList
                .Where(x => x.priceList.Exists(z => z.type == IAPExchangeObject.ExchangeType.RealMoney))
