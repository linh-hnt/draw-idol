...
//Create your obfuscator secrets using Window > Unity IAP > Receipt Validation Obfuscator
//before enabling local receipt validation in the next line, or add it to your Project Settings > Player > Other Settings
//under Scripting Define Symbols to keep this setting even when upgrading and overwriting this file 
        ...
        public override void Validate(Product p = null)
        {
            ...
            for (int i = 0; i < products.Length; i++)
            {
                ...
                try
                {
                    ...
                    var prod = products[i];
                    validator.Validate(prod.receipt);
                    UpdateCustomData(null, prod);
                    IAPManager.GetInstance().CompletePurchase(prod.definition.id, prod.transactionID, withEvent, prod);

                    if (IAPManager.isDebug) Debug.Log("Local Receipt Validation passed for: " + prod.definition.id);
                }
                catch (Exception ex)
                {                   
                    #if UNITY_EDITOR
                    ...
                    {
                        IAPManager.GetInstance().CompletePurchase(products[i].definition.id, products[i].transactionID, withEvent);
